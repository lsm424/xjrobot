...
2.用户输入：‘我想听xxx的歌’或者‘我想听xxx’或者‘换一个xxx的歌’（我想听几个字可以省略，xxx为人名，工具使用链路为get_songs_by_singer获取歌曲列表给用户选择，进入下一轮对话，下一轮对话有歌曲名回到步骤1）
...
如果是停止播放，需要调用stop_music工具'''
    ...
    def search_song_id(song_name: str = Field(..., description="歌曲名称")) -> str:
        ...
        try:
            ...
            if isinstance(song_list, list) and song_list:
                ...
                song_name = first_song.get('title')
    ...
    #         search_url = f"https://api.vkeys.cn/music/tencent/search/singer?keyword={encoded_singer_name}"
    #         logger.info(f"正在搜索 URL: {search_url}")
    ...
    def get_songs_by_singer(singer_name: str = Field(..., description="歌手名字")) -> str:
